                ...
                <div class="row mb-2 ps-3 pe-3">
                    <div class="col-md-6 col-12 g-3 form-floating">
                        <select name="software_type" id="software_type" class="form-select form-control">
                            <option {{ old('software_type', $software->position) == 1 ? "selected" : "" }} value="1">{{ config('constants.SOFTWARE_TYPE_PROD_TOOL_NAME') }}</option>
                            <option {{ old('software_type', $software->position) == 2 ? "selected" : "" }} value="2">{{ config('constants.SOFTWARE_TYPE_MESSAGING_NAME') }}</option>
                            <option {{ old('software_type', $software->position) == 3 ? "selected" : "" }} value="3">{{ config('constants.SOFTWARE_TYPE_BROWSER_NAME') }}</option>
                            <option {{ old('software_type', $software->position) == 4 ? "selected" : "" }} value="4">{{ config('constants.SOFTWARE_TYPE_SYSTEM_UTIL_NAME') }}</option>
                            <option {{ old('software_type', $software->position) == 5 ? "selected" : "" }} value="5">{{ config('constants.SOFTWARE_TYPE_PROJECT_SPECIFIC_NAME') }}</option>
                            <option {{ old('software_type', $software->position) == 6 ? "selected" : "" }} value="6">{{ config('constants.SOFTWARE_TYPE_PHONE_DRIVERS_NAME') }}</option>
                        </select>
